...
wanakana._convertFullwidthCharsToASCII = function (string) {
  var char, chars, code, i, j, len1;
  chars = stringToArray(string)
  for (i = j = 0, len1 = chars.length; j < len1; i = ++j) {
    char = chars[i];
    code = char.charCodeAt(0);
...
wanakana._katakanaToHiragana = function (kata) {
  var code, hira, hiraChar, j, kataChar, len1, ref;
  hira = [];
  ref = stringToArray(kata);
...
wanakana._hiraganaToKatakana = function (hira) {
  var code, hiraChar, j, kata, kataChar, len1, ref;
  kata = [];
  ref = stringToArray(hira);
  for (j = 0, len1 = ref.length; j < len1; j++) {
    hiraChar = ref[j];
...
wanakana._romajiToKana = function (roma, options, ignoreCase) {
  ...
  while (cursor < len) {
    ...
    while (chunkSize > 0) {
      ...
      if (indexOf.call(wanakana.FOUR_CHARACTER_EDGE_CASES, chunkLC) >= 0 && (len - cursor) >= 4) {
        chunkSize++;
        chunk = getChunk();
        chunkLC = chunk.toLowerCase();
      } else {
    ...
    if (options.IMEMode && chunkLC.charAt(0) === "n") {
      if (roma.charAt(cursor + 1).toLowerCase() === "y" && wanakana._isCharVowel(roma.charAt(cursor + 2)) === false || cursor === (len - 1) || isKana(roma.charAt(cursor + 1))) {
...
export function isHiragana(input: string) {
  var chars;
...
export function isKana(input: string) {
  var chars;
  chars = stringToArray(input);
...
export function isRomaji(input: string) {
  var chars;
  chars = stringToArray(input);
  return chars.every(function (char) {
    return (!isHiragana(char)) && (!isKatakana(char));
...
export function toHiragana(input: string, options?) {
  if (isRomaji(input)) {
...
export function toRomaji(input: string, options?) {
